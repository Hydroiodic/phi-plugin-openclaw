...
await buildResources({ output: path.join(root, 'repository'), version: '3.20.0', gameVersion: '3.20.0', gameCode: 154, info: await resolveTestInfo() })
const server = await serveRepository(path.join(root, 'repository'))
const previousMirror = process.env.PHI_RESOURCE_BASE_URL, previousVersion = process.env.PHI_RESOURCE_VERSION
process.env.PHI_RESOURCE_BASE_URL = server.url
process.env.PHI_RESOURCE_VERSION = 'latest'
const commands = new Map(), hooks = new Map(), services = []
plugin.register({
  pluginConfig: { channels: ['qqbot'], timeout: 60000 },
  logger: console, runtime: { state: { resolveStateDir: () => root } }, resolvePath: p => p,
  registerCommand: def => commands.set(def.name, def),
  on: (name, fn) => hooks.set(name, fn), registerService: service => services.push(service),
})
const replies = []
async function dispatch(body, overrides = {}) {
  const current = []
  const result = await hooks.get('reply_dispatch')({
    ctx: { Provider: 'qqbot', SenderId: 'smoke-user', AccountId: 'default',
      From: 'qqbot:group:smoke-group', To: 'qqbot:group:smoke-group', ChatType: 'group',
      CommandBody: body, RawBody: body, CommandAuthorized: true, ...overrides },
    sendPolicy: 'allow',
  }, {
    dispatcher: {
      sendFinalReply: p => { current.push(p); replies.push(p); return true },
...
try {
  ...
  const [{ default: getInfo }, { default: Save }, { UserCredentials }, { default: Config }] = await Promise.all([
    import('../model/game/getInfo.js'), import('../model/save/Save.js'),
    import('../model/user/userCredentials.js'), import('../components/Config.js'),
  ])
  ...
  for (const id of Object.keys(getInfo.ori_info)) {
    for (const directory of ['ill', 'illLow', 'illBlur']) fs.copyFileSync(fallback, path.join(artworkSource, directory, id.replace(/\.0$/, '') + '.png'))
  }
  for (const song of Object.values(getInfo.sp_info)) fs.copyFileSync(fallback, path.join(artworkSource, 'SP', song.id.replace(/\.0$/, '') + '.png'))
  ...
  const records = Object.entries(getInfo.ori_info).filter(([, info]) => info.chart.IN).slice(0, 40)
  assert.equal(records.length, 40)
  const save = new Save({
    session: token, global: false, Recordver: 1, gameuser: { avatar: '', background: '', selfIntro: 'OpenClaw synthetic smoke test' },
    playerInfo: {}, saveInfo: { PlayerId: 'OpenClaw Test', modifiedAt: { iso: new Date().toISOString() },
      summary: { rankingScore: 12, challengeModeRank: 315, gameVersion: 154, updatedAt: new Date().toISOString() } },
    gameRecord: Object.fromEntries(records.map(([id], i) => [id, [null, null, { score: i < 3 ? 1000000 : 980000, acc: i < 3 ? 100 : 98, fc: true }]])),
  }, true)
  const original = UserCredentials.prototype.getUpdatedSaveFromLocal
  UserCredentials.prototype.getUpdatedSaveFromLocal = async function () { return { save } }
  try {
    ...
    assert.equal(server.requests.filter(url => url.startsWith('/illustrations/objects/')).length, 1, 'identical artwork is downloaded once across render requests')
    assert.equal(server.requests.some(url => url.startsWith('/illustrations/packages/')), false, 'default rendering downloads individual objects')
    console.log(JSON.stringify({ ok: true, commands: commands.size, stateDir: root, image: images[0], bytes: fs.statSync(images[0]).size }, null, 2))
  } finally { UserCredentials.prototype.getUpdatedSaveFromLocal = original }
} finally {
  for (const service of services) await service.stop()
  await server.close()
  if (previousMirror === undefined) delete process.env.PHI_RESOURCE_BASE_URL; else process.env.PHI_RESOURCE_BASE_URL = previousMirror
  if (previousVersion === undefined) delete process.env.PHI_RESOURCE_VERSION; else process.env.PHI_RESOURCE_VERSION = previousVersion
}

...
export async function buildIllustrations({ input, output, partBytes = 32 * 1024 ** 2 }) {
  ...
  const source = path.resolve(input),
    root = path.join(path.resolve(output), 'illustrations')
  const relative = path.relative(source, root)
  if (!relative || (!relative.startsWith('..' + path.sep) && relative !== '..' && !path.isAbsolute(relative)))
    throw new Error('曲绘输出目录不能位于输入目录内。')
  ...
  try {
    ...
    if (error.code === 'EEXIST') throw new Error('曲绘仓库已有发布任务，请确认任务结束后再构建。')
    throw error
  }
  let stage
  try {
    ...
    for (const name of names) {
      ...
      const hash = sha256(content),
        target = illustrationObjectPath(hash, name)
    ...
    const sums =
      [`${sha256(indexBytes)}  index.json`, ...index.archives.map(archive => `${archive.sha256}  ${archive.path}`)].join('\n') + '\n'
    ...
    return {
      ...
      archives: index.archives.length,

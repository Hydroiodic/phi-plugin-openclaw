...
async function filesIn(root, prefix = '') {
  const files = []
  for (const entry of (await fs.readdir(path.join(root, prefix), { withFileTypes: true })).sort((a, b) =>
...
async function packageFiles(source, kind) {
  ...
  const notices = new Map(
    [
      ['GPL-3.0.txt', 'LICENSE'],
      ['resources-Apache-2.0.txt', 'resources/LICENSE'],
      ['NOTICE.md', 'NOTICE.md'],
  ...
  for (const name of await filesIn(source)) {
    if (name.toLowerCase().startsWith(prefix.toLowerCase())) {
      ...
      const file = path.join(source, name),
        expected = await fs.readFile(original)
...
export async function buildResources({
  output,
  version,
  gameVersion = version?.split('-r')[0],
  gameCode,
  info = process.env.PHI_RESOURCE_SOURCE_DIR,
  illustrations,
  partBytes = 32 * 1024 * 1024,
}) {
  if (!RESOURCE_VERSION.test(version) || !/^\d+\.\d+\.\d+$/.test(gameVersion) || !Number.isSafeInteger(gameCode) || gameCode < 1)
    throw new Error('必须提供有效的资源 version 和 game-code；可选 game-version 应为 x.y.z。')
  if (version.split('-r')[0] !== gameVersion) throw new Error('资源 version 必须以相同的游戏版本命名，如 3.20.0 或 3.20.0-r1。')
  if (!Number.isSafeInteger(partBytes) || partBytes < 1024 || partBytes > 64 * 1024 * 1024)
    throw new Error('分包大小须在 1 KiB 到 64 MiB 之间。')
  ...
  try {
    if (
      ...
      throw new Error('版本目录已存在，禁止覆盖；请增加版本号。')
    await fs.mkdir(path.join(root, 'v'), { recursive: true })
    let previous = { schemaVersion: 1, releases: [] }
    try {
      previous = validateResourceIndex(JSON.parse(await fs.readFile(path.join(root, 'index.json'), 'utf8')))
    } catch (error) {
    ...
    const manifest = {
      ...
      game: { version: gameVersion, code: gameCode },
      packages: {},
    }
    const checksums = []
    for (const [kind, source] of [['song-data', info]]) {
      ...
      if (kind === 'song-data')
        for (const name of ['DLC/', 'oldInfo/'])
          zip.file(name, null, { dir: true, date: zipDate, unixPermissions: 0o040755, createFolders: false })
      async function flush() {
        if (!count) return
        const filename = `${kind}-${String(++part).padStart(3, '0')}.zip`
        const buffer = await zip.generateAsync({
          type: 'nodebuffer',
          compression: kind === 'song-data' ? 'DEFLATE' : 'STORE',
      ...
      await flush()
      manifest.packages[kind] = { archives }
    }
    ...
    const pointer = {
      schemaVersion: 1,
      version,
      manifest: `v/${version}/metadata.json`,
      sha256: manifestHash,
      game: manifest.game,
    ...
    const table = [
      ...
      ...releases.map(item => [item.version, item.game.version, item.game.code, item.packages.join(',')].join('\t')),
    ]
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const { values } = parseArgs({
      options: {
        output: { type: 'string', default: 'dist-resources' },
        version: { type: 'string' },
        'game-version': { type: 'string' },
        'game-code': { type: 'string' },
        info: { type: 'string' },
        illustrations: { type: 'string' },
      },
    })
    const result = await buildResources({
      output: values.output,
      version: values.version,
      gameVersion: values['game-version'],
      gameCode: Number(values['game-code']),
      info: values.info,

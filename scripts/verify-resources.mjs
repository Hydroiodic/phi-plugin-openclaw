...
const parseJson = (bytes, label) => {
  try {
    ...
    throw new IntegrityError(`${label} 不是有效 JSON：${error.message}`)
  }
}
...
class LocalReader {
  ...
  async read(relative, maxBytes) {
    ...
    for (const part of ['', ...relative.split('/')]) {
      ...
      try {
        check(!(await fs.lstat(current)).isSymbolicLink(), `资源不允许符号链接：${relative}`)
      } catch (error) {
    ...
    try {
      ...
      throw new IntegrityError(`缺少文件：${relative}`)
    }
...
class RemoteReader {
  ...
  async read(relative, maxBytes, expectedType) {
    ...
    try {
      ...
      throw new IntegrityError(`请求失败 ${url.href}：${error.message}`)
    }
    ...
    if (expectedType && !contentType.toLowerCase().startsWith(expectedType))
      this.warn(`${relative} 的 Content-Type 是 ${contentType || '未设置'}，建议设置为 ${expectedType}`)
...
function validateManifest(manifest, pointer) {
  ...
  for (const kind of kinds) {
    ...
    pack.archives.forEach((archive, index) => {
      ...
      check(!archivePaths.has(archive.path), `${label} 分包路径重复：${archive.path}`)
...
function decodeName(bytes) {
  try {
    ...
    throw new IntegrityError('ZIP 文件名不是有效 UTF-8')
  }
}
function inspectZip(bytes, archive, kind, seen) {
  ...
  for (let index = 0; index < entries; index++) {
    ...
    check(
      disk === 0 && compressed !== 0xffffffff && uncompressed !== 0xffffffff && localOffset !== 0xffffffff,
      `${archive.path} 的 ZIP64/磁盘字段无效`,
    )
    ...
    const name = decodeName(rawName),
      ...
      normalized = safeRelative(directory ? name.slice(0, -1) : name)
    check(!seen.has(normalized.toLowerCase()), `${archive.path} 与同包其他分包重名：${normalized}`)
    ...
    try {
      ...
      throw new IntegrityError(`${archive.path} 解压失败 ${name}：${error.message}`)
    }
    ...
    if (directory) check(content.length === 0, `${archive.path} 目录项不应有内容：${name}`)
      ...
      if (kind === 'song-data' && /\.(json|csv|ya?ml|txt|md)$/i.test(normalized)) {
        try {
          ...
          throw new IntegrityError(`${archive.path} 文本不是有效 UTF-8：${normalized}`)
        }
      }
    }
...
async function sourceFiles(root, prefix = '', kind = 'song-data') {
  ...
  try {
    ...
    throw new IntegrityError(`无法读取曲目源目录：${directory}`)
  }
...
async function verifyVersion(reader, pointer, log) {
  ...
  for (const [kind, pack] of Object.entries(manifest.packages)) {
    ...
    if (kind === 'song-data') {
      ...
      check(
        names.some(name => name.startsWith('DLC/') && name.endsWith('.json')),
        `${pointer.version}/song-data 的 DLC/ 为空`,
      )
      check(
        names.some(name => name.startsWith('oldInfo/') && name.endsWith('.json')),
        `${pointer.version}/song-data 的 oldInfo/ 为空`,
      )
    }
  }
...
export async function verifyIllustrations(reader, log) {
  ...
  for (const [name, file] of entries) {
    ...
    check(!names.has(name.toLowerCase()), `曲绘文件重复：${name}`)
    names.add(name.toLowerCase())
    check(
      HASH.test(file?.sha256) && Number.isSafeInteger(file.bytes) && file.bytes > 0 && file.bytes <= MAX_FILE,
      `曲绘校验字段无效：${name}`,
    )
  ...
  const sums = new Map(),
    sumsBytes = (await reader.read(prefix + 'SHA256SUMS', MAX_INDEX, 'text/plain')).bytes
  ...
  for (const archive of index.archives) {
    check(
      HASH.test(archive?.sha256) && archive.path === `packages/${archive.sha256}.zip` && !archives.has(archive.path),
      '曲绘分包路径或哈希无效',
    )
...
export async function verifyRepository(target, { strictHttp = false, log = console.log } = {}) {
  ...
  if (/^https?:\/\//i.test(target)) reader = new RemoteReader(target, strictHttp)
    ...
    let root = requestedRoot,
      sourceRoot
    try {
      await fs.access(path.join(root, 'index.json'))
    } catch {
      try {
        await fs.access(path.join(root, 'public', 'index.json'))
        root = path.join(root, 'public')
      } catch {
        throw new IntegrityError(`在 ${root} 或其 public/ 下找不到 index.json`)
      }
    }
    const sourceCandidate = path.join(root === requestedRoot ? path.dirname(root) : requestedRoot, 'song-data')
    try {
      if ((await fs.stat(sourceCandidate)).isDirectory()) sourceRoot = sourceCandidate
    } catch {}
  ...
  const index = parseJson(indexResult.bytes, 'index.json'),
    latest = parseJson(latestResult.bytes, 'latest.json')
  check(
    index?.schemaVersion === 1 && Array.isArray(index.releases) && index.releases.length > 0 && index.releases.length <= 10000,
    'index.json 的 schemaVersion 或 releases 无效',
  )
  const versions = new Set()
  index.releases.forEach((pointer, position) => {
    validatePointer(pointer, `index.json.releases[${position}]`)
    check(!versions.has(pointer.version), `index.json 版本重复：${pointer.version}`)
    versions.add(pointer.version)
    if (position)
      check(compareVersions(index.releases[position - 1].version, pointer.version) > 0, 'index.json 必须按资源版本从新到旧严格排序')
  })
  ...
  const expectedTab =
    [
      'version\tphigros\tcode\tpackages',
      ...index.releases.map(item => [item.version, item.game.version, item.game.code, item.packages.join(',')].join('\t')),
    ].join('\n') + '\n'
  ...
  for (const pointer of index.releases) {
    try {
      ...
      errors.push(error)
      log(`✗ ${pointer.version}：${error.message}`)
    }
  }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  try {
    await verifyRepository(positionals[0] || process.cwd(), { strictHttp })
  } catch (error) {
    console.error(`\n校验失败：${error.message}`)
    process.exitCode = 1
  }

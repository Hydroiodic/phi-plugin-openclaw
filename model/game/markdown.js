...
/** Queue all quick menus after the command's main replies.
 * @param {botEvent} e @param {string} markdown
 * @param {{command:string,label:string}[]} commands @param {string} errorMessage */
...
export async function sendQuickCommands(e, commands, title = '快捷操作') {
    ...
    if (!markdown) return
    deferQuickMarkdown(e, markdown, commands, '[phi-plugin] 快捷操作 Markdown 发送失败')
}
...
export async function sendQuickCommandSections(/** @type {any} */ e, /** @type {any[]} */ sections, title = '快捷操作') {
    if (!isOfficialBot(e) || !Config.getUserCfg('config', 'LetterMarkdown')) return
...
/** 帮助页：按功能类别提供最常用的入口。 */
export function helpQuickCommands(commandHead = 'phi') {
    return [
...
export function userSettingQuickCommands(commandHead = 'phi') {
    return [
...
export function userSettingQuickCommandSections(commandHead = 'phi') {
    const command = (/** @type {string} */ setting, /** @type {number} */ value) => pageCommand(commandHead, `myset ${setting} ${value}`)
    return [
        {
            title: '主题风格',
            commands: [
...
/** API 用户设置页。 */
export function apiSettingQuickCommands(commandHead = 'phi') {
    return [
...
/** API 帮助页。 */
export function apiHelpQuickCommands(commandHead = 'phi') {
    return [
...
/** Bot 全局设置页。 */
export function configQuickCommands(commandHead = 'phi') {
    return [
...
export function scoreQuickCommands(commandHead = 'phi') {
    return [
        { command: pageCommand(commandHead, 'b27'), label: 'B27成绩' },
        { command: pageCommand(commandHead, 'p30'), label: 'P30成绩' },
        { command: pageCommand(commandHead, 'info'), label: '个人信息' },
...
/** 更新存档结果页。 */
export function updateQuickCommands(commandHead = 'phi') {
    return [
        { command: pageCommand(commandHead, 'b27'), label: 'B27成绩' },
        { command: pageCommand(commandHead, 'p30'), label: 'P30成绩' },
        { command: pageCommand(commandHead, 'info'), label: '个人信息' },
...
/** 成绩筛选页。 */
export function userListQuickCommands(commandHead = 'phi') {
    return [
...
/** 存档历史页。 */
export function historyQuickCommands(commandHead = 'phi') {
    return [
...
/** B19 衍生分析页。 */
export function b19AnalysisQuickCommands(commandHead = 'phi') {
    return [
        { command: pageCommand(commandHead, 'b27'), label: 'B27成绩' },
        { command: pageCommand(commandHead, 'p30'), label: 'P30成绩' },
...
/** 单曲成绩页。 */
export function singleScoreQuickCommands(commandHead = 'phi') {
    return [
        { command: pageCommand(commandHead, 'list'), label: '成绩筛选' },
        { command: pageCommand(commandHead, 'b27'), label: 'B27成绩' },
        { command: pageCommand(commandHead, 'info'), label: '个人信息' },
...
/** 推分建议页。 */
export function suggestQuickCommands(commandHead = 'phi') {
    return [
...
/** 章节成绩页。 */
export function chapterQuickCommands(commandHead = 'phi') {
    return [
...
/** 定数成就页。 */
export function achievementQuickCommands(commandHead = 'phi') {
    return [
...
/** 绑定/更新页。 */
export function sessionQuickCommands(commandHead = 'phi') {
    return [
        { command: pageCommand(commandHead, 'update'), label: '更新存档' },
        { command: pageCommand(commandHead, 'bind qrcode'), label: '扫码绑定' },
...
/** 排行榜页。 */
export function rankQuickCommands(commandHead = 'phi') {
    return [
...
export async function sendMarketQuickCommands(e, themes, pagination = {}) {
    ...
    if (!markdown) return
    deferQuickMarkdown(e, markdown, marketQuickCommands(themes, pagination), '[phi-plugin][主题市场] Markdown 发送失败')
}

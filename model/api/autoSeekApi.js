...
export class AutoSeekApi {
  ...
  constructor({ retryDelayMs = 30_000 } = {}) {
    if (!Number.isSafeInteger(retryDelayMs) || retryDelayMs < 1 || retryDelayMs > 2 ** 31 - 1) throw new RangeError('Invalid API retry delay')
  ...
  seekApi() {
    ...
    this.retryPromise = this.retryLoop().catch(() => {
  ...
  close() {
    ...
    this.closing = Promise.allSettled([this.statusPromise, this.retryPromise]).then(() => { this.waitApi = false })
    return this.closing
  }
}

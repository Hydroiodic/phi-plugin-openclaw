...
import { configPath } from '../filesystem/path.js'
...
function writeIdentityAtomic(identity) {
    if (
        ...
        return
    const target = path.join(configPath, 'config.yaml')
...
export class BotApiAuth {
    ...
    async initialize(pluginVersion = SUPPORTED_API_VERSION) {
        ...
        this.initializing = this.initializeOnce(pluginVersion).finally(() => {
            this.initializing = null
        })
    ...
    async register(pluginVersion = SUPPORTED_API_VERSION) {
        ...
        try {
            response = await axios.post(
                `${APIBASEURL}/bot-clients/register`,
                JSON.stringify({

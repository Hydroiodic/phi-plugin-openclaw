...
export function analyzeSaveHistory(history) {
    ...
    const mostPlayedSongsTop3 = topNNumberMap(songCount, 3).map(x => ({
        id: safeText(getInfo.idgetsong(/** @type {idString} */ (x.key)) || x.key),
        count: safeText(x.value),
    }))
    ...
    const rksMaxUpDay =
        rksMaxUp && rksMaxUp.value > 0 ? { day: safeText(rksMaxUp.key), delta: fmtSigned(rksMaxUp.value, 4) } : { day: '--', delta: '--' }

    const rksMaxDownDay =
        rksMaxDown && rksMaxDown.value < 0
    ...
    const dataMaxUpDownDay = {
        up:
            dataMaxUp && dataMaxUp.value > 0
                ? { day: safeText(dataMaxUp.key), deltaBytes: fmtBytes(dataMaxUp.value) }
                : { day: '--', deltaBytes: fmtBytes(0) },
        down:
            dataMaxDown && dataMaxDown.value < 0
                ? { day: safeText(dataMaxDown.key), deltaBytes: fmtBytes(dataMaxDown.value) }
                : { day: '--', deltaBytes: fmtBytes(0) },
    }

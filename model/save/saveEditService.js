...
const SETTING_LABELS = {
    chordSupport: '多押提示',
    fcAPIndicator: 'FC/AP 指示器',
    enableHitSound: '打击音效',
    lowResolutionMode: '低分辨率模式',
    deviceName: '设备名',
    bright: '背景亮度',
    musicVolume: '音乐音量',
    effectVolume: '界面音效音量',
...
const formatMoney = money =>
    money
        .map((value, index) => (value ? `${value}${MONEY_UNITS[index]}` : ''))
        .filter(Boolean)
        .reverse()
        .join(' ') || '0KB'
...
const formatScore = record =>
    record ? `${record.score} / ${record.acc.toFixed(2)}%${record.score === MAX_SCORE ? ' φ' : record.fc ? ' FC' : ''}` : '无成绩'
...
export class SaveEditService {
    ...
    constructor({
        ...
        saveManager = isGlobal => new SaveManager(isGlobal),
        uploader = isGlobal => new CloudSaveUploader({ isGlobal }),
    ...
    exclusive(userId, work) {
        ...
        settled.then(() => {
            if (this.locks.get(userId) === settled) this.locks.delete(userId)
        })
        return next
    }
    ...
    fetch(userId) {
        return this.exclusive(userId, async () => {
            ...
            const { bytes } = await this.transport.request(
                save.gameFile.url,
                { method: 'GET' },
                { maxBytes: CLOUD_SAVE_LIMITS.maxArchiveBytes },
            )
            ...
            this.remember({
                userId,
                isGlobal,
                expiresAt: this.now() + WORKING_COPY_TTL,
                zip: bytes,
                ...opened,
                cloud: {
                    saveId: save.objectId,
                    userObjectId: playerInfo.objectId,
                    fileId: save.gameFile.objectId,
                    modifiedAt: new Date(save.modifiedAt.iso).toISOString(),
    ...
    overview(copy) {
        ...
        return [
            ...
            ...['EZ', 'HD', 'IN', 'AT'].map(
                (level, index) => `${level}：Clear ${summary.cleared[index]}，FC ${summary.fullCombo[index]}，φ ${summary.phi[index]}`,
            ),
            copy.readOnlyReason
                ? `注意：${copy.readOnlyReason}`
                : changes.length
                  ? `未上传的修改：${changes.length} 项`
                  : '没有未上传的修改',
        ].join('\n')
    }
            ...
            switch (section) {
                ...
                case 'profile': {
                    ...
                    return [
                        ...
                        `头像：${user.avatar}`,
                        `背景：${user.background}`,
                        `显示玩家 ID：${user.showPlayerId ? '是' : '否'}`,
                    ].join('\n')
                }
                case 'settings':
                    return Object.entries(SETTING_LABELS)
                        .map(([key, label]) => `${label}（${key}）：${formatValue(current.settings.values[key])}`)
                        .join('\n')
                case 'progress': {
                    const progress = current.gameProgress.values
                    return [
                        ...
                        ...Object.entries(progress)
                            .filter(([key]) => !['money', 'challengeModeRank'].includes(key))
                            .map(([key, value]) => `${key}：${formatValue(value)}`),
                    ].join('\n')
                }
                case 'records':
                    return this.listRecords(copy, { song, level, offset, limit })
                case 'changes': {
                    const changes = this.changes(copy)
                    return changes.length ? changes.join('\n') : '没有未上传的修改。'
                }
                default:
                    throw new SaveEditError(`未知的存档分区：${section}`)
            }
        })
    }
    ...
    listRecords(copy, { song, level, offset, limit }) {
        ...
        for (const item of copy.current.gameRecord.songs) {
            ...
            item.levels.forEach((record, index) => {
                ...
                rows.push({
                    text: `${catalog.songName(item.id) || item.id} [${item.id}] ${LEVEL_NAMES[index]}${difficulty ? ` ${difficulty}` : ''}：${formatScore(record)}`,
                    rks: difficulty ? rksOf(record, difficulty) : 0,
                })
            })
        }
        ...
        const start = Math.max(0, Math.floor(offset)),
            size = Math.min(Math.max(1, Math.floor(limit)), 50)
        return [
            `共 ${rows.length} 条成绩，按单曲 RKS 排序，显示第 ${start + 1}~${Math.min(start + size, rows.length)} 条：`,
            ...rows.slice(start, start + size).map(row => row.text),
        ].join('\n')
    }
        ...
        for (const [key, value] of Object.entries(profile)) {
            switch (key) {
                case 'selfIntro':
                    ...
                    break
                case 'avatar':
                    if (typeof value !== 'string' || !this.catalog().isAvatar(value))
                        throw new SaveEditError(`头像「${value}」不存在，请使用游戏内已有的头像名称。`)
                    values.avatar = value
                    break
                case 'background': {
                    ...
                    if (!name)
                        throw new SaveEditError(
                            ids.length ? `「${value}」匹配到多首曲目，请写完整曲名。` : `没有找到曲目「${value}」，无法设为背景。`,
                        )
                ...
                case 'showPlayerId':
                    ...
                    break
                default:
                    throw new SaveEditError(`不支持修改个人资料字段 ${key}。`)
            }
        }
    }
    ...
    editRecords(copy, next, records) {
        ...
        for (const change of records) {
            ...
            if (ids.length !== 1) {
                throw new SaveEditError(
                    ids.length
                        ? `「${change.song}」匹配到多首曲目：${ids
                              .slice(0, 5)
                              .map(id => `${catalog.songName(id) || id} [${id}]`)
                              .join('、')}，请用曲目 ID 指定。`
                        : `没有找到曲目「${change.song}」。`,
                )
            }
            ...
            if (catalog.difficulty(id, LEVEL_NAMES[level]) === undefined)
                throw new SaveEditError(`${id} 没有 ${LEVEL_NAMES[level]} 难度的谱面。`)
            const old = song?.levels[level]
            const record = {
                score: change.score ?? old?.score,
                acc: change.acc ?? old?.acc,
                fc: change.fc ?? (change.score !== undefined || change.acc !== undefined ? undefined : old?.fc),
            }
            if (!Number.isInteger(record.score) || record.score < 0 || record.score > MAX_SCORE)
                throw new SaveEditError('分数必须是 0~1000000 的整数。')
            if (typeof record.acc !== 'number' || !Number.isFinite(record.acc) || record.acc < 0 || record.acc > 100)
                throw new SaveEditError('acc 必须是 0~100 的数字。')
    ...
    changes(copy) {
        ...
        const catalog = this.catalog()
        const lines = []
        for (const [section, label] of /** @type {const} */ ([
            ['user', '个人资料'],
            ['settings', '设置'],
            ['gameProgress', '进度'],
        ])) {
        ...
        for (const id of new Set([...before.keys(), ...after.keys()])) {
            for (let level = 0; level < LEVEL_NAMES.length; level++) {
                const old = before.get(id)?.levels[level] ?? null,
                    now = after.get(id)?.levels[level] ?? null
    ...
    confirm(userId, code) {
        return this.exclusive(userId, async () => {
            ...
            if (
                String(code || '')
                    .trim()
                    .toUpperCase() !== pending.code
            )
                throw new SaveEditError('确认码不正确。')
            this.pending.delete(userId)
            const { copy } = pending
            const { sessionToken, isGlobal } = await this.session(userId)
            ...
            if (
                save.objectId !== copy.cloud.saveId ||
                save.gameFile?.objectId !== copy.cloud.fileId ||
                playerInfo.objectId !== copy.cloud.userObjectId
            ) {
                this.copies.delete(userId)
            ...
            try {
                ...
                await uploader
                    .pointSaveAt(sessionToken, {
                        ...target,
                        fileId: copy.cloud.fileId,
                        summary: copy.cloud.summary,
                        modifiedAt: copy.cloud.modifiedAt,
                    })
                    .catch(() => {
                        throw new SaveEditError(
                            `上传后校验失败，自动回滚也失败了。原存档已备份到插件数据目录（${path.basename(backup)}），请联系机器人管理员恢复。`,
                        )
                    })
    ...
    async backup(copy) {
        ...
        const file = path.join(
            directory,
            `${copy.cloud.modifiedAt.replace(/[:.]/g, '-')}-${copy.cloud.fileId.replace(/[^A-Za-z0-9]/g, '')}.zip`,
        )
    ...
    buildSummary(copy) {
        ...
        for (const id of new Set([...before.keys(), ...after.keys()])) {
            for (let level = 0; level < 4; level++) {
                const old = before.get(id)?.levels[level],
                    now = after.get(id)?.levels[level]
                if (JSON.stringify(old ?? null) === JSON.stringify(now ?? null)) continue
                recordsChanged = true
                for (const [record, sign] of /** @type {const} */ ([
                    [old, -1],
                    [now, 1],
                ])) {
                    if (!record) continue
    ...
    rankingScore(songs) {
        ...
        const top = (/** @type {number[]} */ list, /** @type {number} */ count) =>
            list
                .sort((a, b) => b - a)
                .slice(0, count)
...
function editSettings(next, settings) {
    ...
    for (const [key, value] of Object.entries(settings)) {
        ...
        if (rule.maxLength && (typeof value !== 'string' || [...value].length > rule.maxLength))
            throw new SaveEditError(`${key} 必须是不超过 ${rule.maxLength} 字的文本。`)
    ...
    for (const [key, value] of Object.entries(progress)) {
        if (key === 'money') {
            if (
                !Array.isArray(value) ||
                value.length !== 5 ||
                value.some((amount, index) => !Number.isInteger(amount) || amount < 0 || amount > (index < 4 ? 1023 : 0x3fff))
            ) {
            ...
            const color = Math.floor(Number(value) / 100),
...
export async function openArchive(zip, summary) {
    ...
    for (const name of REQUIRED) {
        ...
        if (!entry || entry.data.length < 17 || (entry.data.length - 1) % 16 !== 0)
            throw new SaveEditError('云存档内容损坏，请在游戏内重新同步后再试。')
        versions[name] = entry.data[0]
        try {
            plain[name] = decryptSaveBytes(entry.data.subarray(1))
        } catch {
            throw new SaveEditError('云存档解密失败，请在游戏内重新同步后再试。')
        }
    }
    ...
    const lossless =
        encodeGameRecord(baseline.gameRecord).equals(plain.gameRecord) &&
...
function defaultCatalog() {
    return {
        ...
        difficulty: (id, level) =>
            getInfo.info(/** @type {idString} */ (id), true)?.chart?.[/** @type {allLevelKind} */ (level)]?.difficulty,

...
import { pluginResources, stateRoot, pluginRoot } from '../filesystem/path.js'

export const themesDir = stateRoot === pluginRoot ? path.join(pluginResources, 'themes') : path.join(stateRoot, 'themes')
if (stateRoot !== pluginRoot) {
    fs.mkdirSync(themesDir, { recursive: true, mode: 0o700 })
    const bundled = path.join(pluginResources, 'themes', 'milthm')

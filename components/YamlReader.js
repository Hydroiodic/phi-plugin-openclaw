...
import atomicFile from '../model/filesystem/atomicFile.js'
...
export default class YamlReader {
    ...
    initYaml() {
        // Reject malformed YAML before any mutation can rewrite the document.
        const document = YAML.parseDocument(fs.readFileSync(this.yamlPath, 'utf8'))
        if (document.errors.length) throw new Error('配置文件 YAML 格式无效，请先修复后再保存。')
    ...
    save() {
        ...
        atomicFile.write(this.yamlPath, yaml)
        this.isSave = true
    }

...
/** @param {botEvent} e */
const choiceKey = e => `${e.chatId || e.group_id || 'private'}:${e.user_id}`
...
export default class phiPluginBase extends HostPlugin {
     ...
     */
    constructor({ name = 'your-plugin', dsc = '无', handler, namespace, event = 'message', priority = 5000, task = undefined, rule = [] }) {
        super({
            name,
            dsc,
            handler,
     ...
     */
    async choseMutiNick(e, idList, options, callback) {
        ...
        if (idList.length === 1) {
            await callback(e, idList[0], options)
            return
        }
        send.send_with_At(e, fCompute.mutiNick(idList))
        const userId = choiceKey(e)
    ...
    async mutiNick() {
        ...
        if (!num) {
            ...
            await state?.callback(this.e, ids[num - 1], state.options)
            return true
        }
    }
    ...
    async getMicInfoFromMsg(e, fncName, params = [], options, callback) {
        ...
        if (ids.length > 1) {
            ...
            await callback(e, id, { ...optObj, ...options })
        }
    }
}

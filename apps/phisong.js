...
import getChartTag from '../model/game/getChartTag.js'
...
export class phisong extends phiPluginBase {
    constructor() {
        super({
            ...
            rule: [
                {
                    reg: `^[#/](${Config.getUserCfg('config', 'cmdhead')})(\\s*)(曲(?!绘)|song).*$`,
                    fnc: 'song',
                },
                {
                ...
                {
                    reg: `^[#/](${Config.getUserCfg('config', 'cmdhead')})(\\s*)(com(?!ment)|计算).*$`,
                    fnc: 'comrks',
                },
                {
    ...
    async song(e) {
        ...
        if (ids[0]) {
            if (!ids[1]) {
                ...
                await this.choseMutiNick(e, ids, { page, addComment }, async (e, id, options) => {
    ...
    async search(e) {
        ...
        const patterns = {
            ...
            difficulty: {
                ...
                predicate: (item, bottom, top) =>
                    item?.['chart']
                        ? Object.values(item['chart']).some(level => bottom <= level['difficulty'] && level['difficulty'] <= top)
                        : false,
            },
            combo: {
                regex: /(combo|cmb|物量|连击)([\s:：,，/|~是为]*)([0-9]+(\s*-\s*[0-9]+)?)/,
                predicate: (item, bottom, top) =>
                    item?.['chart'] ? Object.values(item['chart']).some(level => bottom <= level['combo'] && level['combo'] <= top) : false,
            },
        }
        ...
        for (const key in patterns) {
            ...
            if (match) {
                let matchStr = match[0].replace(
                    /((bpm|difficulty|dif|难度|定级|定数|combo|cmb|物量|连击)([\s:：,，/|~是为]*))(\d)/g,
                    '$1 $4',
                )
                matchStr = matchStr.replace(/((bpm|difficulty|dif|难度|定级|定数|combo|cmb|物量|连击)([\s:：,，/|~是为]*))|\s/g, '')
                const [bottom, top] = matchStr.includes('-')
                    ? matchStr.split('-').sort((a, b) => Number(a) - Number(b))
                    : [matchStr, matchStr]
    ...
    async ill(e) {
        ...
        if (ids[0]) {
            if (!ids[1]) {
                ...
                await this.choseMutiNick(e, ids, {}, async (e, id) => {
    ...
    async randmic(e) {
        ...
        if (rank[0]) {
            if (rank[0].includes('+')) {
                if (rank[1]) {
                    send.send_with_At(
                        e,
                        `含有 '+' 的难度不支持指定范围哦！\n/${Config.getUserCfg('config', 'cmdhead')} rand <定数>+ <难度(可多选)>`,
                        true,
                    )
                    return true
                } else {
                ...
                if (rank[1]) {
                    ...
                    if (isNaN(tb) || isNaN(tt)) {
                        send.send_with_At(
                            e,
                            `${rank[0]} - ${rank[1]} 不是一个定级范围哦\n/${Config.getUserCfg('config', 'cmdhead')} rand <定数1> - <定数2> <难度(可多选)>`,
                            true,
                        )
                        return true
                    }
        ...
        if (!songsname[0]) {
            send.send_with_At(
                e,
                `未找到 ${bottom} - ${top} 的 ${isask[0] ? `${Level[0]} ` : ''}${isask[1] ? `${Level[1]} ` : ''}${isask[2] ? `${Level[2]} ` : ''}${isask[3] ? `${Level[3]} ` : ''}谱面QAQ!`,
            )
            return true
        }
    ...
    async alias(e) {
        ...
        if (ids[0]) {
            if (!ids[1]) {
                ...
                await this.choseMutiNick(e, ids, {}, (e, id) => makeNickMsg(e, id))
            }
        } else {
    ...
    async recallComment(e) {
        ...
        getComment.del(commentId) ? send.send_with_At(e, `删除成功！`) : send.send_with_At(e, `删除失败QAQ！`)
    }
    ...
    async addtag(e) {
        ...
        const operation = e.msg.match(/\b(addtag|subtag|retag)\b/i)?.[1].toLowerCase()
        const input = e.msg.replace(/^[#/].*?\b(addtag|subtag|retag)\s*/i, '').trim()
        const match = input.match(/^(\S[\s\S]*?)\s+(\S+)$/)
        if (!match || !operation) {
            send.send_with_At(e, '用法：/phi addtag <曲名> [EZ|HD|IN|AT] <标签>；subtag 反对，retag 撤回自己的投票。')
            return true
        }
        const tag = match[2]
        if (Array.from(tag).length > 6 || /[\u0000-\u001f\u007f]/.test(tag)) {
            send.send_with_At(e, '标签请使用 1 至 6 个字符，不要包含控制字符。')
            return true
        }
        const event = platform.cloneEvent(e, { msg: `/phi ${operation} ${match[1]}` })
        await this.getMicInfoFromMsg(event, /^[#/].*?\b(addtag|subtag|retag)\s*/i, ['rank'], {}, async (selectedEvent, id, { rank }) => {
            const success =
                operation === 'retag'
                    ? getChartTag.cancel(id, tag, rank, String(selectedEvent.user_id))
                    : getChartTag.add(id, tag, rank, operation === 'addtag', String(selectedEvent.user_id))
            send.send_with_At(selectedEvent, success ? '本地标签投票已更新。' : '标签保存失败，请检查磁盘空间和目录权限。')
        })
        return true
    }
...
async function songInfo(page, addComment, id, e) {
    ...
    if ((await Config.getUserCfg('config', 'allowComment')) && (addComment || page)) {
        ...
        let commentData
        if (await canUseApi(e)) {
            commentData = (await makeRequest.getCommentsBySongId({ song_id: infoData.id }, { event: e })) || []

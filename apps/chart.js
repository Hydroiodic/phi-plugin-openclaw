...
export class phihelp extends phiPluginBase {
    ...
    async tag(e) {
        ...
        await this.getMicInfoFromMsg(e, /[#/](.*?)(tag)(\s*)/, ['rank'], {}, async (e, id, optObj) => {
    ...
    async settag(e) {
        ...
        if (!categorySelection) {
            await this.getMicInfoFromMsg(e, /[#/](.*?)(settag)(\s*)/, ['rank'], { sessionToken, tagTree }, async (e, id, optObj) => {
        ...
        if (!childSelections.length) {
            ...
            await this.getMicInfoFromMsg(cleanEvent, /[#/](.*?)(settag)(\s*)/, ['rank'], { sessionToken }, async (e, id, optObj) => {
                const timeout = Number(Config.getUserCfg('config', 'mutiNickWaitTimeOut')) || 0
        ...
        await this.getMicInfoFromMsg(
            cleanEvent,
            /[#/](.*?)(settag)(\s*)/,
            ['rank'],
            { selectedTags, sessionToken },
            async (e, id, optObj) => {
...
async function getChartImg(e, id, options) {
    ...
    if (await canUseApi(e, 'scoreStatistics')) {
        ...
        const apiChartTag = await makeRequest.getChartsTagbySongRankWithTree({ song_id: info.id, rank }, { event: e })
        if (apiChartTag) {
    ...
    const chartInfo = {
        ...
        tip: '',
        chartLength: `${Math.floor((chart.maxTime || 0) / 60)}:${Math.floor((chart.maxTime || 0) % 60)
            .toString()
            .padStart(2, '0')}`,
        words,
        wordsMaxValue,
...
async function getChartTags(e, id, options) {
    ...
    let usersVote = null
    usersVote = await credentials.getChartsUsersVote([{ song_id: info.id, rank: [rank] }], { ignoreUnboundError: true })
    if (!usersVote) {
        usersVote = []
    }

...
export class PhiUpdate extends phiPluginBase {
    constructor() {
        super({ name: 'phi-update', priority: 1009, rule: [
            { reg: '^[/#]phi\\s*(强制|qz)?(更新|gx)$', fnc: 'update' },
            { reg: '^[/#]phi\\s*(下载|更新|gx|down|up)\\s*(曲绘|ill)$', fnc: 'illustrations' },
        ] })
    }
    async update() {
        return this.reply('请在终端使用 openclaw plugins update phi-plugin-openclaw 更新插件，再重启 Gateway。本地链接安装请更新源码并安装依赖。更新游戏存档请使用 /phi update。')
    }
    async illustrations() {

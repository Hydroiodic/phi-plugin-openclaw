...
export default new (class getQRcode {
    ...
    validateRequest(request, maxSeconds = 600) {
        ...
        if (
            ...
            !data.device_code.trim() ||
            data.device_code.length > 4096 ||
            /** @type {any} */ (request).success === false ||
            !Number.isSafeInteger(data.expires_in) ||
            data.expires_in <= 0 ||
            typeof data.qrcode_url !== 'string' ||
            data.qrcode_url.length > 4096
        )
            throw new Error('扫码登录响应格式无效。')
        let url
        try {
            url = new URL(data.qrcode_url)
        } catch {
            throw new Error('扫码登录链接无效。')
        }
        if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password) throw new Error('扫码登录链接无效。')
        return {
            seconds: Math.min(data.expires_in, maxSeconds),
    ...
    async getSessionToken(result, useGlobal = false, options = {}) {
        if (
            !result?.data ||
            ['kid', 'mac_key', 'scope', 'access_token'].some(
                key => typeof (/** @type {any} */ (result.data)[key]) !== 'string' || !(/** @type {any} */ (result.data)[key]),
            )
        )
            throw new Error('扫码授权结果格式无效。')
        const profile = await TapTapHelper.getProfile(result.data, useGlobal, options)

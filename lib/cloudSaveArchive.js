...
export const CLOUD_SAVE_LIMITS = Object.freeze({
...
export class CloudSaveArchive {
    ...
    constructor(limits = {}) {
        ...
        for (const limit of Object.values(this.limits))
            if (!Number.isSafeInteger(limit) || limit <= 0) throw new TypeError('云存档限制参数无效。')
    }
    ...
    inspect(bytes) {
        ...
        if (
            ...
            throw new Error('云存档 ZIP 文件数量或结构无效。')
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (!/^[A-Za-z0-9_.-]+$/.test(name) || name === '.' || name === '..' || entries.has(name))
                throw new Error('云存档 ZIP 包含重复或不安全的文件名。')
            ...
            total += unpacked
            if (unpacked > this.limits.maxEntryBytes || total > this.limits.maxTotalBytes)
                throw new Error('云存档解压数据超出安全大小限制。')
            if (bytes.readUInt16LE(cursor + 8) & 1 || ![0, 8].includes(bytes.readUInt16LE(cursor + 10)))
                throw new Error('云存档 ZIP 压缩方式无效。')
            cursor = next
        }
    ...
    async readEntries(bytes) {
        ...
        try {
            ...
            throw new Error('云存档解压失败、数据损坏或超出安全限制。')
        }
    }

...
export default class ByteReader {
    ...
    constructor(data, position = 0) {
        ...
        this.data = typeof data === 'string' ? Buffer.from(data, 'hex')
    ...
    assertAvailable(size) {
        if (!Number.isSafeInteger(size) || size < 0 || !Number.isSafeInteger(this.position)
            || this.position < 0 || size > this.data.length - this.position) {
    ...
    getByte() { return this.readNumber(1, position => this.data.readUInt8(position)) }
    getShort() { return this.readNumber(2, position => this.data.readUInt16LE(position)) }
    getInt() { return this.readNumber(4, position => this.data.readInt32LE(position)) }
    getFloat() { return this.readNumber(4, position => this.data.readFloatLE(position)) }

    /** @param {number} value */
    putByte(value) { this.writeNumber(1, position => this.data.writeUInt8(value, position)) }
    /** @param {number} value */
    putShort(value) { this.writeNumber(2, position => this.data.writeUInt16LE(value, position)) }
    /** @param {number} value */
    putInt(value) { this.writeNumber(4, position => this.data.writeInt32LE(value, position)) }
    /** @param {number} value */
    putFloat(value) { this.writeNumber(4, position => this.data.writeFloatLE(value, position)) }
    ...
    getBytes() { return this.readBytes(() => this.getByte()) }
    getString() { return this.readBytes(() => this.getVarInt()).toString('utf8') }
    skipString() { this.readBytes(() => this.getVarInt()) }
    ...
    insertBytes(bytes) { this.replaceBytes(0, bytes) }

    /** @param {number} length @param {Uint8Array} bytes */
    replaceBytes(length, bytes) {
        this.assertAvailable(length)
        this.data = Buffer.concat([
            this.data.subarray(0, this.position), bytes, this.data.subarray(this.position + length),
        ])
    }
}

        ...
        for (const field of ENTRY_LAYOUTS[name]) {
            switch (field.type) {
                case 'flags': {
                    ...
                    field.bits.forEach((bit, index) => {
                        values[bit] = Util.getBit(flags, index)
                    })
                    break
                }
                case 'byte':
                    values[field.name] = reader.getByte()
                    break
                case 'short':
                    values[field.name] = reader.getShort()
                    break
                case 'varint':
                    values[field.name] = reader.getVarInt()
                    break
                case 'float':
                    values[field.name] = reader.getFloat()
                    break
                case 'string':
                    values[field.name] = reader.getString()
                    break
                case 'varints':
                    values[field.name] = Array.from({ length: field.count }, () => reader.getVarInt())
                    break
                case 'levelStats': {
                    values.cleared = []
                    values.fullCombo = []
    ...
    for (const field of ENTRY_LAYOUTS[name]) {
        switch (field.type) {
            case 'flags': {
                ...
                field.bits.forEach((bit, index) => {
                    flags = Util.modifyBit(flags, index, Boolean(values[bit]))
                })
                writer.byte(flags)
                break
            }
            case 'byte':
                writer.byte(values[field.name], field.name)
                break
            case 'short':
                writer.short(values[field.name], field.name)
                break
            case 'varint':
                writer.varint(values[field.name], field.name)
                break
            case 'float':
                writer.float(values[field.name], field.name)
                break
            case 'string':
                writer.string(values[field.name], field.name)
                break
            case 'varints': {
                const list = values[field.name]
                if (!Array.isArray(list) || list.length !== field.count)
                    throw new RangeError(`${field.name} 必须包含 ${field.count} 个数字`)
    ...
    try {
        ...
        while (reader.remaining() > 0) {
            ...
            for (let level = 0; level < LEVEL_NAMES.length; level++) {
                levels.push(
                    Util.getBit(present, level)
                        ? { score: reader.getInt(), acc: reader.getFloat(), fc: Util.getBit(fcFlags, level) }
                        : null,
                )
            }

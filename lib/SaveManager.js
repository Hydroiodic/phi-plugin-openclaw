...
export default class SaveManager {
    ...
    async latestSave(session) {
        ...
        const saves = (await this.saveArray(session, playerInfo.objectId)).filter(
            item => item?.gameFile && typeof item.gameFile.url === 'string' && Number.isFinite(new Date(item.modifiedAt?.iso).getTime()),
        )

...
export default new class TapTapHelper {
    ...
    async requestLoginQrCode(permissions = ['public_profile'], useGlobalEndpoint = false, options = {}) {
        ...
        params.append('client_id', useGlobalEndpoint ? 'kviehleldgxsagpozb' : 'rAK3FfdieFob2Nn8Am');
        ...
        const data = /** @type {Record<string, unknown>} */ (await cloudTransport.json(endpoint, {
            method: 'POST',
    ...
    async checkQRCodeResult(data, useGlobalEndpoint = false, options = {}) {
        ...
        try {
            return await cloudTransport.json(endpoint, {
                method: 'POST',
                body: params, signal: options.signal,
     ...
     * @returns {Promise<any>}
     */
    async getProfile(token, useGlobalEndpoint = false, timestamp = 0, options = {}) {
        ...
        if (useGlobalEndpoint) {
            url = `${this.ApiHost}/account/profile/v1?client_id=kviehleldgxsagpozb`;
        } else {
        ...
        return cloudTransport.json(url, {
            method: 'GET',
            headers: { Authorization: authorizationHeader }, signal: options.signal,
        }, { timeoutMs: options.timeoutMs });
    }
}()

...
export class CloudTransport {
    ...
    async request(url, options = {}, policy = {}) {
        const maxBytes = policy.maxBytes ?? this.maxBytes,
        ...
        try {
            ...
            throw new CloudTransportError('CLOUD_URL', '云端地址无效。')
        }
        ...
        let reader
        const abortedError = () =>
            new CloudTransportError(
                timeout.signal.aborted ? 'CLOUD_TIMEOUT' : 'CLOUD_ABORTED',
    ...
    async json(url, options, policy) {
        ...
        try {
            ...
            throw new CloudTransportError('CLOUD_JSON', '云端返回的数据格式无效，请稍后重试。')
        }
    }
}

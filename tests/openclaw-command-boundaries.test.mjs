...
test('pending group context cannot capture credential commands or host commands', async t => {
  ...
  class Interaction extends adapter.PluginBase {
    constructor() {
      super({ rule: [{ reg: '^/phi choose$', fnc: 'choose' }] })
    }
    async choose(e) {
      this.e = e
      this.setContext('answer', true)
      await adapter.reply(e, '选择')
    }
    async answer(e) {
      inputs.push(e.msg)
    }
  ...
  const event = msg =>
    adapter.fromContext({ channel: 'qqbot', senderId: 'test', isGroup: true, conversationId: 'group', commandBody: msg }, payload =>
...
test('zero-priority commands precede default priority and invalid handlers fail at startup', async t => {
  ...
  class Default extends adapter.PluginBase {
    constructor() {
      super({ rule: [{ reg: '^/phi test$', fnc: 'run' }] })
    }
    async run() {
      observed.push('default')
      return true
    }
  }
  class First extends Default {
    constructor() {
      super()
      this.priority = 0
    }
  ...
  class Invalid extends adapter.PluginBase {
    constructor() {
      super({ rule: [{ reg: '^/phi test$', fnc: 'absent' }] })
    }
  }

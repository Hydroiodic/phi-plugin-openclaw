...
const authorization = {
  success: true,
  data: { kid: 'synthetic-kid', mac_key: 'synthetic-key', scope: 'public_profile', access_token: 'synthetic-access' },
}
...
const stageOf = url =>
  String(url).includes('/device/code')
    ? 'request'
    : String(url).includes('/account/profile/')
      ? 'profile'
      : String(url).endsWith('/users')
        ? 'login'
        : 'poll'
...
function setup(t, signal) {
  ...
  t.mock.method(UserCredentials.prototype, 'bindLocallyWithSessionToken', async (value, global) => {
    bindings.push({ value, global })
    return null
  })
  const event = commandBody =>
    adapter.fromContext({ channel: 'qqbot', senderId: 'synthetic-user', commandBody, signal }, payload => {
...
test('actual QR binding retries a network-null result and pending HTTP response before binding both regions', async t => {
  ...
  t.mock.method(globalThis, 'fetch', async url => {
    switch (stageOf(url)) {
      case 'request':
        return response({ success: true, data: qrData })
      case 'poll': {
        polls++
      ...
      case 'profile':
        return response({ data: { openid: 'synthetic-openid' } })
      case 'login':
  ...
  assert.deepEqual(fixture.bindings, [
    { value: token, global: false },
    { value: token, global: true },
  ])
...
test('QR binding rejects invalid expiry, URL and success data without polling forever or binding old credentials', async t => {
  ...
  t.mock.method(globalThis, 'fetch', async url => {
    switch (stageOf(url)) {
      ...
      case 'poll':
        polls++
        return response(malformedSuccess ? { success: true, data: {} } : authorization)
      default:
        profiles++
        throw new Error('must not exchange malformed authorization')
    }
  })
  ...
  for (const override of [
    { qrcode_url: 'javascript:alert(1)' },
    { qrcode_url: 'https://user:secret@example.invalid' },
    { device_code: '' },
  ]) {
...
test('QR polling expires at the original bounded deadline after repeated null responses', async t => {
  ...
  t.mock.method(getQRcode, 'wait', async ms => {
    now += ms
  })
  t.mock.method(globalThis, 'fetch', async url => {
...
for (const phase of ['request', 'poll', 'wait', 'profile', 'login']) {
  test(`QR cancellation interrupts ${phase}, clears cache and never binds`, async t => {
    const controller = new AbortController(),
      fixture = setup(t, controller.signal),
      started = Promise.withResolvers()
    if (phase === 'wait') {
      const wait = getQRcode.wait
      t.mock.method(getQRcode, 'wait', async (ms, signal) => {
        started.resolve()
        return wait.call(getQRcode, ms, signal)
      })
    }
    t.mock.method(globalThis, 'fetch', async (url, options) => {
      ...
      switch (stage) {
        case 'request':
          return response({ success: true, data: { ...qrData, interval: 30 } })
        case 'poll':
          return response(phase === 'wait' ? { success: false, data: { error: 'authorization_pending' } } : authorization)
        case 'profile':
          return response({ data: { openid: 'synthetic-openid' } })
        case 'login':
          return response({ sessionToken: token })

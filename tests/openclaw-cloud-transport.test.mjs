...
test('cloud transport bounds advertised and streamed bodies, cancels readers and sanitizes errors', async () => {
  ...
  await assert.rejects(
    badJson.json('https://example.invalid'),
    error => error.code === 'CLOUD_JSON' && !String(error).includes('private-token'),
  )
  const badNetwork = new CloudTransport({
    fetcher: async () => {
      throw new Error('Authorization: private-network-token')
    },
  })
  await assert.rejects(
    badNetwork.json('https://example.invalid?private-query'),
    error => error.code === 'CLOUD_NETWORK' && !String(error).includes('private'),
  )
...
test('TapTap polling preserves non-200 pending responses and bounded failures remain retryable', async t => {
  t.mock.method(
    globalThis,
    'fetch',
    async () => new Response(JSON.stringify({ success: false, data: { error: 'authorization_pending' } }), { status: 400 }),
  )
...
test('PhigrosUser downloads and parses a bounded complete snapshot; failures preserve existing records', async t => {
  const files = {
    gameProgress: encrypt(Buffer.alloc(17)),
    user: encrypt(Buffer.alloc(4)),
    settings: encrypt(Buffer.alloc(26)),
    gameRecord: encrypt(Buffer.from([0])),
  }
  ...
  user.getSaveInfo = async () => {
    user.saveInfo = { gameFile: { url: 'https://example.invalid/save' }, summary: { saveVersion: 2 } }

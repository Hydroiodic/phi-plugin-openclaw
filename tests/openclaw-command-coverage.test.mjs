...
const cases = {
  ...
  aliasProposal: {
    propose: 'alias submit Credits | test',
    mine: 'alias mine',
    publicList: 'alias public',
    appeal: 'alias appeal 1',
    vote: 'alias vote 1',
    unvote: 'alias unvote 1',
  },
  apiSetting: {
    setApiToken: 'setApiToken test',
    tokenList: 'tkls',
    auth: 'auth',
  ...
  phisong: {
    ...
    live: 'live',
    table: 'table 15',
    difHis: 'difHistory Credits',
    comment: 'comment Credits',
    recallComment: 'recmt 1',
    myComment: 'mycmt',
    addtag: 'addtag Credits',
    newNotice: 'newnotice',
  },
...
const aliases = {
  ...
  'RankList.rankfind': ['查询排名 1'],
  'aliasProposal.propose': ['alias 提案 Credits | 测试'],
  'aliasProposal.mine': ['alias 我的'],
  'aliasProposal.publicList': ['alias 公审'],
  'aliasProposal.appeal': ['alias 申诉 1'],
  'aliasProposal.vote': ['alias 投票 1'],
  'aliasProposal.unvote': ['alias 撤票 1'],
  ...
  'b19.suggest': ['推分', '推分建议'],
  'b19.achievement': ['ahv'],
  'botClient.resetApiBot': ['重置API Bot身份'],
  'botClient.claimLink': ['获取Bot认领链接'],
  'guessGame.start': ['letter', '开字母', '猜曲绘', '提示猜曲', 'tipgame', 'guess', 'ltr -L 5'],
  'guessGame.getTip': ['提示'],
  'guessGame.ans': ['答案', '结束'],
  'help.help': ['命令', '帮助', '菜单', '说明', '功能', '指令', '使用说明'],
  'help.tkhelp': ['token帮助', 'tokhelp', 'tken说明', 'tk 菜单'],
  'help.apihelp': ['api帮助'],
  'manage.backup': ['backup back'],
  'market.marketPage': ['pr', '上一页', '下一页'],
  'money.sign': ['sign in', '签到', '打卡'],
  'money.tasks': ['我的任务'],
  'money.retask': ['刷新任务'],
  'money.send': ['送 1 2', '转 1 2'],
  'money.theme': ['theme1'],
  'money.jrrp': ['今日人品'],
  'phisong.song': ['曲 Credits'],
  'phisong.search': ['查找 Credits', '检索 Credits'],
  'phisong.setnick': ['setnic Credits', '设置别名 Credits'],
  'phisong.ill': ['曲绘 Credits', 'Ill Credits'],
  'phisong.randmic': ['随机', 'random'],
  'phisong.comrks': ['计算 15 98'],
  ...
  'saveEdit.confirm': ['确认上传 ABC234'],
  'saveEdit.cancel': ['取消上传'],
  'session.bind': ['bind qrcode', 'cnbind qrcode', '绑定 qrcode', 'cn绑定 qrcode', 'gb绑定 qrcode'],
  'session.update': ['更新存档'],
  'session.unbind': ['解绑'],
  'session.getSstk': ['sessiontoken'],
  'setting.showUserSetting': ['mysetting', '用户设置', '个人设置'],
  'setting.set': ['设置'],
  'update.update': ['更新', 'gx', 'qzgx', 'qz更新', '强制gx'],
  'update.illustrations': ['下载曲绘', '更新曲绘', 'gxill', 'downill', 'up ill'],
  'user.info': ['info1', 'info2'],
  'user.lvscore': ['lvscore', 'scolv'],
...
test('every enabled app command reaches its handler through the OpenClaw router', async t => {
  ...
  for (const file of (await fs.readdir(new URL('../apps/', import.meta.url))).filter(name => name.endsWith('.js'))) {
    ...
    apps[key] = class extends adapter.PluginBase {
      constructor() {
        super({ rule: instance.rule, priority: instance.priority })
      }
    }
    for (const rule of instance.rule)
      apps[key].prototype[rule.fnc] = async () => {
        observed.push(`${key}.${rule.fnc}`)
        return true
      }
    assert.deepEqual(
      [...new Set(instance.rule.map(rule => rule.fnc))].sort(),
      Object.keys(cases[key] || {}).sort(),
      `${key}: incomplete command inventory`,
    )
  ...
  for (const [app, handlers] of Object.entries(cases))
    for (const [handler, example] of Object.entries(handlers)) {
  ...
  for (const [body, expected] of [
    ['/gbbind qrcode', 'session.bind'],
    ['/cnbind qrcode', 'session.bind'],
    ['/pgr b30', 'b19.b19'],
    ['/PGRB30', 'b19.b19'],
    ['/屁股肉帮助', 'help.help'],
    ['杠phi啊比三零', 'b19.arcgrosB19'],
    ['phisign', 'money.sign'],
    ['phi task', 'money.tasks'],
    ['phitheme1', 'money.theme'],
    ['/开a', 'guessGame.reveal'],
    ['#open A', 'guessGame.reveal'],
    ['/phi tipgame', 'guessGame.start'],
    ['/phi guess', 'guessGame.start'],
    ['/phi qzgx', 'update.update'],
    ['/phi backup back', 'manage.backup'],
    ['/phi p40', 'b19.p30'],
    ['/phi x30', 'b19.p30'],
    ['/phi FC30', 'b19.p30'],
    ['/phi rks', 'b19.b19'],
    ['/phi 单曲成绩2 Credits', 'b19.singlescore'],
    ['/phi 下一页', 'market.marketPage'],
    ['/phi setApiToken test', 'apiSetting.setApiToken'],
  ]) {
    assert.equal(isPhigrosCommand(body), true, body)
    await check(body, expected)
  }
  for (const [expected, examples] of Object.entries(aliases))
  ...
  for (const [app, handlers] of Object.entries(cases))
    for (const [handler, example] of Object.entries(handlers)) {
      if (
        (app === 'guessGame' && ['guess', 'reveal'].includes(handler)) ||
        ['help', 'send', 'a b30'].includes(example) ||
        example.startsWith('send ')
      )
        continue
      const body = `/${example}`
      assert.equal(isPhigrosCommand(body), true, body)
  ...
  t.diagnostic(
    `${Object.keys(apps).length} apps, ${Object.values(originals).reduce((n, app) => n + app.rule.length, 0)} rules, ${count} routed command cases`,
  )
})

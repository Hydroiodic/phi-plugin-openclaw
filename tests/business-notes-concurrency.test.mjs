...
test('ordered user locks support reentry, release on failure and avoid opposite-order deadlocks', async () => {
    const lock = new UserDataLock(), observed = []
    await Promise.all([
        lock.run(['a', 'b'], async () => {
            observed.push('first')
            await lock.run(['a'], async () => { observed.push('nested'); await tick() })
        }),
        lock.run(['b', 'a'], () => observed.push('second')),
    ])
    assert.deepEqual(observed, ['first', 'nested', 'second'])
    await assert.rejects(lock.run(['a'], () => { throw new Error('synthetic failure') }), /synthetic/)
    await lock.run(['a'], () => observed.push('recovered'))
    assert.equal(lock.pending.size, 0)
    await assert.rejects(lock.run(['a'], () => lock.run(['b'], () => {})), /嵌套/)
    assert.equal(lock.pending.size, 0)
})
...
test('detached async work cannot reuse ownership after its original lock has been released', async () => {
    ...
    let wake, release, started, detached, ran = false
    const trigger = new Promise(resolve => { wake = resolve })
    await lock.run(['a'], () => {
        detached = trigger.then(() => lock.run(['a'], () => { ran = true }))
    })
...
test('a recipient write failure compensates the sender and unlocks both accounts', async t => {
    ...
    let fail = true
    t.mock.method(getNotes, 'putNotesData', (id, data) => {
        if (id === 'target' && fail) { fail = false; return false }
...
test('concurrent task refresh awards a completed task only once and preserves settings updates', async t => {
    ...
    const results = await Promise.all([
        ...
        getNotes.update('player', data => { data.showB30Analysis = false }),
    ])
    assert.equal(results.slice(0, 5).reduce((sum, value) => sum + value[1], 0), 25)
...
test('simultaneous real sign handlers pay the daily award once alongside incoming transfers', async t => {
    ...
    t.mock.method(send, 'send_with_At', async (_event, message) => { messages.push(message); return true })
    ...
    t.after(() => { getInfo.noticeJson = oldNotice })

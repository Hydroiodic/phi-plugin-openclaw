...
test('official and SP artwork always use shared references; only explicit custom songs may override images', t => {
  const official = 'Credits.Frums.0',
    special = 'Anomaly.0',
    custom = 'Custom.0'
  const ori = getInfo.ori_info,
    sp = getInfo.sp_info
  t.after(() => {
    getInfo.ori_info = ori
    getInfo.sp_info = sp
  })
  ...
  let enabled = 0
  t.mock.method(Config, 'getUserCfg', name =>
    name === 'config' ? enabled : { [custom]: { id: custom, illustration: 'https://custom.example/image.png' } },
  )
...
test('updateHistory uploads only the caller history and does not claim success before upload completes', async t => {
  ...
  t.mock.method(UserCredentials.prototype, 'getSessionToken', async function () {
...
test('updateHistory respects disabled API, missing credentials, empty history and API failures', async t => {
  ...
  t.mock.method(UserCredentials.prototype, 'uploadHistory', async () => {
    uploads++
    return null
  })
  const command = new phihelp()
  ...
  assert.equal(
    messages.some(message => /已上传/.test(message)),
    false,
  )
})
...
test('local tag commands add, change and cancel only the caller vote through real handlers', async t => {
  ...
  const messages = [],
    originalConfig = Config.getUserCfg
...
test('restore selection snapshots file ordering and rejects blank, fractional and expired choices', async t => {
  ...
  t.mock.method(fs, 'readdirSync', (file, options) =>
    file === backupPath ? names.map(name => ({ name, isFile: () => true })) : originalReadDir(file, options),
  )
  t.mock.method(send, 'send_with_At', async (_event, message) => messages.push(message))
  t.mock.method(getBackup, 'restore', async file => {
    restored.push(file)
  })

...
import {
  buildQuickCommandMarkdown, buildQuickCommandSectionsMarkdown, buildMarketQuickMarkdown,
  sendQuickCommands, sendQuickCommandSections, sendMarketQuickCommands,
} from '../model/game/markdown.js'

const commands = [{ command: '/b30', label: '成绩' }]
const sections = [{ title: '成绩', commands }, { title: '设置', commands }]
...
test('all quick menus keep command tables without decorative horizontal rules', () => {
  for (const markdown of [buildQuickCommandMarkdown(commands), buildQuickCommandSectionsMarkdown(sections),
...
test('all quick menu senders defer until image and subsequent text replies finish', async () => {
  const previous = getPlatformAdapter(), previousConfig = Config.getUserCfg
  const adapter = createPlatform()
  setPlatformAdapter(adapter)
  Config.getUserCfg = (_name, key) => key === 'LetterMarkdown' ? true : 'phi'
  try {
    for (const send of [e => sendQuickCommands(e, commands), e => sendQuickCommandSections(e, sections),
      e => sendMarketQuickCommands(e, themes)]) {
      const replies = []
      const e = event(adapter, async payload => { replies.push(payload) })
  ...
  } finally { Config.getUserCfg = previousConfig; setPlatformAdapter(previous); adapter.close() }
})

test('clones share delivery order while unrelated requests remain independent', async () => {
  const adapter = createPlatform(), delivered = []
  let release
  ...
  try {
    ...
    const other = event(adapter, payload => { delivered.push(payload.text) })
...
test('cancelled replies do not send a queued image or footer', async () => {
  const adapter = createPlatform(), replies = [], controller = new AbortController()
  try {
...
test('concurrent flush calls wait for the same drain and never advance menus early', async () => {
  ...
  try {
    ...
    adapter.afterReplies(e, async () => { menuStarted = true; await adapter.reply(e, 'menu') })
    const first = adapter.flush(e), second = adapter.flush(adapter.cloneEvent(e))
...
test('a failed footer drains its fire-and-forget work exactly once', async () => {
  ...
  try {
    ...
    adapter.afterReplies(e, async () => { await adapter.reply(e, 'failed menu') })
    adapter.afterReplies(e, async () => { await adapter.reply(e, 'must not run') })
...
test('closing is idempotent, prevents new events and cancels queued delivery', async () => {
  ...
  adapter.afterReplies(e, async () => { await adapter.reply(e, 'menu') })
  adapter.close()

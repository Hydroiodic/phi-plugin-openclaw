  ...
  return {
      ...
      songs: [
        {
          ...
          fcFlags: 0b100,
          levels: [null, null, { score: 1000000, acc: 100, fc: true }, { score: 950000, acc: Math.fround(98.5), fc: false }, null],
        },
    ...
    gameProgress: {
      values: {
        ...
        money: [100, 200, 3, 0, 0],
        unlockFlagOfSpasmodic: 1,
        unlockFlagOfIgallta: 1,
        unlockFlagOfRrharil: 1,
        flagOfSongRecordKey: 0,
        randomVersionUnlocked: 3,
        chapter8UnlockBegin: true,
        chapter8UnlockSecondPhase: false,
        chapter8Passed: false,
        chapter8SongUnlocked: 7,
      },
      // 最高位是插件不认识的标记，必须原样保留
      flagBytes: [0b10000110, 0b1],
      tail: TAIL,
    },
    user: {
      values: { showPlayerId: true, selfIntro: '你好', avatar: 'Introduction', background: 'Glaciaxion' },
      flagBytes: [1],
      tail: Buffer.alloc(0),
    },
    settings: {
      values: {
        ...
        bright: 1,
        musicVolume: 1,
        effectVolume: Math.fround(0.5),
        hitSoundVolume: 1,
        soundOffset: 0,
        noteScale: 1,
      },
      flagBytes: [0b0111],
      tail: Buffer.from([1, 2]),
    },
    summary: {
      values: {
        saveVersion: 6,
        challengeModeRank: 345,
        rankingScore: Math.fround(0.5),
        gameVersion: 120,
        avatar: 'Introduction',
        cleared: [1, 0, 1, 1],
        fullCombo: [0, 0, 1, 0],
...
async function buildZip(content = sampleContent()) {
  ...
  return new CloudSaveArchive().write(
    Object.entries(plain).map(([name, data]) => ({
      name,
      compression: 'DEFLATE',
      data: Buffer.concat([Buffer.of(versions[name]), encryptSaveBytes(data)]),
    })),
  )
}
...
const catalog = () => ({
  findSongs: query =>
    ({ glaciaxion: ['Glaciaxion.SunsetRay.0'], other: ['Other.Composer.0'], same: ['A.0', 'B.0'] })[query.toLowerCase()] || [],
  ...
  difficulty: (id, level) =>
    ({ 'Glaciaxion.SunsetRay.0': { EZ: 1, HD: 4, IN: 12, AT: 14 }, 'Other.Composer.0': { EZ: 2, HD: 6 } })[id]?.[level],
...
async function fakeCloud(t, { verifyFails = false } = {}) {
  ...
  const cloud = {
    save: {
      objectId: 'save1',
      summary: encodeSummary(sampleContent().summary),
      modifiedAt: { iso: '2026-09-01T00:00:00.000Z' },
      gameFile: { objectId: 'file-old', url: 'https://cloud.test/old' },
    },
    uploads: [],
    rollbacks: [],
  }
  ...
  const service = new SaveEditService({
    ...
    uploader: () =>
      /** @type {any} */ ({
        ...
        async pointSaveAt(session, target) {
          cloud.rollbacks.push(target)
        },
      }),
...
test('save codec round-trips every entry and keeps unknown bits and trailing bytes', () => {
  ...
  assert.throws(
    () => encodeEntry('user', { ...content.user, values: { ...content.user.values, selfIntro: 'x'.repeat(20000) } }),
    RangeError,
  )
})
...
test('save edits are validated atomically and uploads need the confirmation code', async t => {
  ...
  assert.match(
    await service.read('me', { section: 'records', song: 'glaciaxion' }),
    /Glaciaxion \[Glaciaxion\.SunsetRay\.0\] IN 12：1000000/,
  )

  await assert.rejects(
    service.edit('me', { profile: { selfIntro: 'changed' }, records: [{ song: 'glaciaxion', level: 'AT', score: 999999, acc: 50 }] }),
    /不可能同时出现/,
  )
  ...
  assert.deepEqual(
    await fs.readFile(
      path.join(
        backupRoot,
        'me',
        backups.find(name => name.endsWith('.zip')),
      ),
    ),
    zip,
  )
...
test('uploads stop when the cloud save changed and roll back when verification fails', async t => {
  ...
  assert.deepEqual(
    broken.cloud.rollbacks.map(target => target.fileId),
    ['file-old'],
  )
})
...
test('save tools only act for the requester in their own isolated direct session', async () => {
  ...
  for (const sessionKey of [
    ...
    'agent:main:qqbot:group:alice',
    'agent:main:qqbot:direct:bob',
    'agent:main:telegram:direct:alice',
    'agent:main:qqbot:bot2:direct:alice',
    undefined,
  ]) {
  ...
  const calls = []
  const tools = createSaveTools({ ...base, sessionKey: 'agent:main:qqbot:bot1:direct:alice' }, async (...args) => {
    calls.push(args)
    return 'ok'
  })
  assert.deepEqual(
  ...
  const failing = createSaveTools({ ...base, sessionKey: 'agent:main:qqbot:bot1:direct:alice' }, async () => {
...
test('uploader follows the LeanCloud and Qiniu flow without leaking the session token', async () => {
  ...
  assert.deepEqual(
    requests.map(request => `${request.method} ${request.url.replace(/^https:\/\/[^/]+/, '')}`),
    [
      'POST /1.1/fileTokens',
  ...
  const callbacks = []
  const unsupported = new CloudSaveUploader({

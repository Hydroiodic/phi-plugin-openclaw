...
import {
  ...
  validateIllustrationIndex,
  parseIllustrationChecksums,
...
async function fixture(t) {
  ...
  const input = path.join(root, 'illustrations'),
  ...
  const config = { resourceBaseUrl: server.url },
  ...
  return {
    ...
    create,
    warnings,
    reference: illustrationReference('ill', '曲目 A.png'),
    index: JSON.parse(await fs.readFile(path.join(output, 'illustrations/index.json'), 'utf8')),
  }
}

test('shared catalog is version-free, content-addressed, deduplicated and reproducible', async t => {
  const f = await fixture(t)
  assert.equal(f.built.files, 2)
  assert.equal(f.built.objects, 1)
...
test('catalog rejects unsafe names, object hashes, sizes and duplicate checksums', async t => {
  ...
  for (const mutate of [
    x => {
      x.files['../bad.png'] = x.files['ill/曲目 A.png']
    },
    x => {
      x.files['ill/DUPLICATE.png'] = x.files['ill/duplicate.png']
    },
    x => {
      x.files['ill/duplicate.png'].path = 'v/3.20.0/image.png'
    },
    x => {
      x.files['ill/duplicate.png'].bytes = 0
    },
    x => {
      x.files['ill/duplicate.png'].bytes++
    },
    x => {
      x.archives[0].sha256 = 'bad'
    },
    x => {
      x.archives = []
    },
    ...
    mutate(index)
    assert.throws(() => validateIllustrationIndex(index))
  }
...
test('lazy fetch deduplicates simultaneous aliases, verifies bytes and reuses offline cache', async t => {
  ...
  assert.equal(
    f.server.requests.some(url => url.endsWith('.zip')),
    false,
  )
...
test('corrupt cache refetches; corrupt downloads never become renderable images', async t => {
  const f = await fixture(t),
    repo = f.create(),
    local = await repo.resolve(f.reference)
...
test('corrupt catalog and ZIP are rejected without replacing cached images', async t => {
  const f = await fixture(t),
    repo = f.create(),
    local = await repo.resolve(f.reference)
  const catalog = path.join(f.output, 'illustrations/index.json'),
    original = await fs.readFile(catalog)
  ...
  assert.equal(await fs.stat(local).catch(() => null), null)
  assert.equal(
    (await fs.readdir(repo.root)).some(name => name.startsWith('.download-')),
    false,
  )
})
...
test('game upgrades reuse the shared artwork without embedding it in version manifests', async t => {
  const f = await fixture(t),
    info = path.join(f.root, 'song-data')
  await fs.mkdir(path.join(info, 'DLC'), { recursive: true })
  await fs.mkdir(path.join(info, 'oldInfo'))
  ...
  assert.notEqual(
    new ResourceRepository({ config: { resourceBaseUrl: f.server.url + 'mirror/' }, dataRoot: f.dataRoot, env: {} }).root,
    f.cacheRoot,
  )
})
...
test('a new song refreshes the shared catalog without a game-version dependency', async t => {
  ...
  await buildIllustrations({ input: f.input, output: f.output })
  assert.deepEqual(await Promise.all(['new.png', 'newer.png'].map(name => repo.resolve(illustrationReference('ill', name)))), [
    local,
    local,
  ])
  assert.equal(f.server.requests.filter(url => url.includes('/objects/')).length, 1)
...
test('preparation preserves prototypes, aliases and cyclic render data without mutating input', async t => {
  ...
  const item = Object.assign(new Record(), { illustration: f.reference, buffer: png }),
    input = { item, items: [item] }
  input.self = input
  const output = await repo.prepare(input)
  assert.notEqual(output, input)
  assert.equal(output.self, output)
  assert.equal(output.item, output.items[0])
  assert.equal(output.item.name, 'record')
  assert.equal(output.item.buffer, png)
  assert.equal(input.item.illustration, f.reference)
...
test('direct image replies receive verified local bytes, not opaque references or remote URLs', async t => {
  const f = await fixture(t),
    platform = new OpenClawPlatform({ dataRoot: path.join(f.root, 'platform') })
  platform.illustrations = f.create()
...
test('cache symlinks, source symlinks and corrupt published hash objects fail safely', async t => {
  ...
  assert.equal(
    sha256(await fs.readFile(path.join(f.output, 'illustrations/index.json'))),
    sha256(Buffer.from(JSON.stringify(f.index, null, 2) + '\n')),
  )
})
...
test('closing the image service aborts an in-flight transfer and prevents later use', async t => {
  ...
  const pending = repo.resolve(f.reference)
...
test('standalone verifier checks ZIPs, objects, source hashes and missing files', async t => {
  ...
  const reader = {
    ...
    remote: false,
    async read(relative) {
      return { bytes: await fs.readFile(path.join(f.output, relative)) }
    },
  }

...
const history = value => ({ version: 3, scoreHistory: {}, rks: [{ date: `2026-01-0${value}T00:00:00Z`, value }], data: [], challengeModeRank: [] })
...
async function fixture(t, credentialStore = { setSessionToken: async () => {} }) {
    ...
    const service = new BackupRestoreService({ saveRoot: path.join(directory, 'save'), pluginDataRoot: path.join(directory, 'plugin'), credentialStore })
...
test('backup restore awaits file and credential writes, merges history, and keeps newer saves', async t => {
    ...
    const { service } = await fixture(t, { setSessionToken: async (user, value) => {
    ...
    let completed = false
    const pending = service.apply(plan).then(() => { completed = true })
    ...
    assert.deepEqual(JSON.parse(await fs.readFile(historyFile, 'utf8')).rks.map(item => item.value), [1, 2])
    assert.equal(JSON.parse(await fs.readFile(path.join(service.pluginDataRoot, 'user.json'), 'utf8')).money, 42)
    assert.equal((await fs.readdir(path.dirname(saveFile))).some(name => name.endsWith('.tmp')), false)
})
...
test('backup reader enforces compressed-expansion budgets and entry count', async () => {
    const zip = await JSZip.loadAsync(await new JSZip().file('one', 'a'.repeat(4096)).file('two', 'b'.repeat(4096)).generateAsync({ type: 'nodebuffer', compression: 'DEFLATE' }))
...
test('backup restore propagates credential persistence failures instead of reporting success', async t => {
    const { service } = await fixture(t, { setSessionToken: async () => { throw new Error('synthetic database failure') } })
...
test('fresh installations can create private, complete backups without user data directories', async t => {
    ...
    const result = await getBackup.backup({ msg: '/phi backup' }, {
        saveRoot: path.join(directory, 'absent-save'), pluginDataRoot: path.join(directory, 'absent-plugin'), outputRoot, includeThemes: false,
    })
...
test('backup collection skips unexpected files and symlinks instead of archiving their targets', async t => {
    const { directory } = await fixture(t)
    const saveRoot = path.join(directory, 'save'), pluginDataRoot = path.join(directory, 'plugin'), outputRoot = path.join(directory, 'backups')
    ...
    assert.equal(Object.keys(zip.files).some(name => name.includes('unexpected')), false)
})

test('archive-stream failures leave no final ZIP or unfinished temporary output', async t => {
    const { directory } = await fixture(t)
    const pluginDataRoot = path.join(directory, 'plugin'), outputRoot = path.join(directory, 'backups')
    await fs.mkdir(pluginDataRoot)
    ...
    t.mock.method(send, 'send_with_At', async () => true)
    await assert.rejects(getBackup.backup({ msg: '/phi backup' }, {
        saveRoot: path.join(directory, 'absent-save'), pluginDataRoot, outputRoot, includeThemes: false,
    }), { code: 'ENOENT' })

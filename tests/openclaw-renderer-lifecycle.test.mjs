...
test('template cleanup owns generated files only and preserves source and unrelated HTML', async () => {
  ...
  const source = path.join(dir, 'source.art'),
    unrelated = path.join(dir, 'external.html')
  try {
...
test('browser installs coalesce per cache directory and retry after failures', async () => {
  ...
  const first = resolver.resolve('/tmp/phi-a'),
    shared = resolver.resolve('/tmp/phi-a'),
    second = resolver.resolve('/tmp/phi-b')
  await new Promise(resolve => setImmediate(resolve))
  assert.equal(requests.length, 2)
  gate.resolve()
  assert.deepEqual(await Promise.all([first, shared, second]), [
    '/tmp/phi-a/browser/chrome',
    '/tmp/phi-a/browser/chrome',
    '/tmp/phi-b/browser/chrome',
  ])
...
test('browser detection rejects non-executable candidates and unsupported automatic platforms', async () => {
  let installs = 0
  const resolver = new BrowserResolver({
    candidates: () => ['/directory', '/good/chrome'],
    executable: file => file === '/good/chrome',

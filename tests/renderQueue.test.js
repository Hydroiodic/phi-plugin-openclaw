...
import { createSettingsForm, createSettingsSchemas } from '../components/settings/form.js'
...
test('shared settings expose the queue limit and settings form validates and saves it', () => {
    assert.equal(shared.defaults.renderQueueLimit, 32)
    assert.ok(shared.editable.some(item => item.key === 'renderQueueLimit'))
    assert.ok(createSettingsSchemas().some(item => item.field === 'renderQueueLimit'))

  ...
  function scan(dir) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) scan(file)
        ...
        for (const match of fs.readFileSync(file, 'utf8').matchAll(/\bplatform\.([A-Za-z_$][\w$]*)\s*\(/g)) required.add(match[1])
    }
  }
  try {
...
test('tag command event clones retain identity, shared reply tracking and delivery', async () => {
  const adapter = createPlatform(),
    replies = []
  try {
    const original = adapter.fromContext({ channel: 'qqbot', senderId: 'test', commandBody: '/phi settag Credits #tag' }, reply =>
...
test('canonical command forms and group sender isolation', async () => {
  ...
  const event = overrides =>
    adapter.fromContext(
      { channel: 'qqbot', accountId: 'bot', senderId: 'alice', isGroup: true, from: 'qqbot:group:group1', ...overrides },
      () => {},
    )
  try {
    const a = event({}),
      b = event({ senderId: 'bob' })
...
test('credentials stay private; commands outrank game listeners; async replies drain; contexts are scoped', async () => {
  const adapter = createPlatform()
  const replies = []
  const event = (commandBody, overrides = {}) =>
    adapter.fromContext({ channel: 'qqbot', senderId: 'alice', isGroup: false, from: 'dm', commandBody, ...overrides }, p => {
      replies.push(p)
      return true
    })
  class Scores extends adapter.PluginBase {
    constructor() {
      super({
        rule: [
          { reg: '^/phi b30$', fnc: 'score' },
          { reg: '^/phi choose$', fnc: 'choose' },
        ],
      })
    }
    async score(e) {
    ...
    async answer(e) {
      adapter.reply(e, `选择:${e.msg}`)
      this.finish('answer')
    }
  }
  class Games extends adapter.PluginBase {
    constructor() {
      super({ rule: [{ reg: '^.*$', fnc: 'guess' }] })
    }
    async guess() {
      return false
    }
  ...
  class Bind extends adapter.PluginBase {
    constructor() {
      super({ rule: [{ reg: '^/phi (bind|绑定).*$', fnc: 'bind' }] })
    }
    async bind(e) {
      await adapter.reply(e, 'bound')
    }
  }
  const router = createRouter({ b19: Scores, guessGame: Games, session: Bind }, adapter)
  const run = async e => {
    const result = await router.dispatch(e)

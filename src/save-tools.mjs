...
const NOT_PRIVATE =
  '存档工具只能在与用户一对一、并且按用户隔离的私聊会话中使用。请让用户私聊机器人；' +
...
export function resolveRequester(ctx) {
  ...
  if (
    !peer ||
    keyChannel !== channel.toLowerCase() ||
    peer !== sender.toLowerCase() ||
    (keyAccount !== undefined && keyAccount !== account.toLowerCase())
  )
    return { error: NOT_PRIVATE }
...
const TOOLS = {
  ...
  phigros_save_read: {
    label: 'Phigros 查看存档',
    description:
      '查看已读取的存档：overview 概览、profile 个人资料、settings 设置、progress 进度与 Data、records 成绩（可按曲名和难度筛选）、changes 未上传的修改。',
    parameters: {
      type: 'object',
      properties: {
    ...
    parameters: {
      ...
      properties: {
        ...
        settings: {
          type: 'object',
          properties: {
            chordSupport: { type: 'boolean' },
            fcAPIndicator: { type: 'boolean' },
            enableHitSound: { type: 'boolean' },
            lowResolutionMode: { type: 'boolean' },
            deviceName: { type: 'string' },
            bright: { type: 'number', minimum: 0, maximum: 1 },
            musicVolume: { type: 'number', minimum: 0, maximum: 1 },
            effectVolume: { type: 'number', minimum: 0, maximum: 1 },
            hitSoundVolume: { type: 'number', minimum: 0, maximum: 1 },
        ...
        progress: {
          ...
          properties: {
            money: {
              ...
              minItems: 5,
              maxItems: 5,
              description: 'Data：[KB, MB, GB, TB, PB]',
            },
...
export async function runSaveTool(service, name, userId, params, { delivery } = {}) {
  switch (name) {
    case 'phigros_save_fetch':
      return service.fetch(userId)
    case 'phigros_save_read':
      return service.read(userId, params)
    case 'phigros_save_edit':
      return service.edit(userId, params)
    case 'phigros_save_upload': {
      const { code, changes } = await service.stage(userId)
      const message = [
        `即将把以下 ${changes.length} 项修改上传到你的 Phigros 云存档：`,
        ...changes,
        '',
        `确认无误请发送：/phi 确认上传 ${code}`,
        '放弃请发送：/phi 取消上传',
    ...
    default:
      throw new Error(`Unknown save tool ${name}`)
  }
}

...
export class PhigrosPlugin {
  ...
  allowed(channel) {
    return !this.config.channels?.length || this.config.channels.includes(channel)
  }
  ...
  ensureRuntime() {
    ...
    if (!this.pending) {
      this.pending = Promise.resolve()
        .then(() => this.createRuntime(this.options))
        .catch(error => {
          this.pending = undefined
          throw error
        })
  ...
  async handleNative(ctx) {
    ...
    const replies = []
    await this.handle(
      { ...ctx, isGroup: ctx.isGroup === true || /(?:^|:)(?:group|channel|guild)(?::|$)/.test(ctx.from || '') },
      payload => {
    ...
    const handled = await this.handle(
      {
        ...
        commandBody: String(ctx.CommandBody ?? ctx.RawBody ?? ''),
        from: ctx.From,
        to: ctx.To,
  ...
  register() {
    ...
    for (const name of NATIVE_COMMANDS) {
      this.api.registerCommand({
        ...
        acceptsArgs: true,
        requireAuth: false,
        handler: ctx => this.handleNative(ctx),
      })
    }
    ...
    if (saveTools) {
      this.api.registerTool(
        ctx =>
          this.allowed(ctx.messageChannel)
            ? createSaveTools(ctx, async (...args) => (await this.ensureRuntime()).runSaveTool(...args), this.api.logger)
            : null,
        { names: SAVE_TOOL_NAMES },
      )
    }

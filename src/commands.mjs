...
const SHORT_HEADS = new Set(
  (
    'bind cnbind gbbind unbind 绑定 cn绑定 gb绑定 解绑 更新存档 update clean sessionToken ' +
    ...
    'phihelp openclawhelp api tk token tok 帮助 命令 菜单 说明 功能 指令 使用说明'
  )
    .toLowerCase()
    .split(' '),
)
...
const PRIVATE_COMMAND =
  /^\/phi\s+(?:(?:cn|gb)?(?:bind|绑定)|unbind|解绑|sessiontoken|auth|setApiToken|tkls|lstk|clearApiData|clean|确认上传|saveupload)/i
...
export function immediateReply({ commandBody, isGroup, channelId, channel, accountId, senderId }) {
  ...
  if (command === '/phi identity')
    return senderId
      ? `管理员配置 ID：${channelId || channel}:${accountId || 'default'}:${senderId}`
      : '无法识别发送者身份，请通过已配置的聊天 channel 使用此命令。'
...
export class CommandRouter {
  constructor(apps, adapter) {
    ...
    this.routes = Object.entries(apps)
      .filter(([, App]) => typeof App === 'function')
      .map(([key, App]) => {
  ...
  async dispatch(e) {
    ...
    const immediate = immediateReply({
      ...
      channel: e.platform,
      accountId: e.self_id,
      senderId: e.rawSenderId,
    })
    if (immediate !== null) {
      await adapter.reply(e, immediate)
      return true
    }
    ...
    if (context) {
      ...
      if (/^(取消|cancel)$/i.test(e.msg)) {
        ...
        context.instance.finish(context.name, context.isGroup)
        await adapter.reply(e, '已取消。')
        return true
      }
      context.instance.e = e
    ...
    for (const listener of [false, true])
      for (const route of this.routes)
        for (const rule of route.rules) {
          ...
          if (
            (ADMIN_MODULES.has(route.key) || rule.permission === 'master' || (route.key === 'setting' && rule.fnc === 'set')) &&
            !e.isMaster
          ) {
            await adapter.reply(e, '此命令仅限插件管理员。使用 /phi identity 查看身份，由部署者配置 admins。')
            return true
          }

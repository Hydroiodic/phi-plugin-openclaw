...
export const REQUIRED_INFO = ['info.csv', 'infolist.json', 'notesInfo.json', 'oldNotesInfo.json', 'spinfo.json', 'nicklist.yaml', 'chaplist.yaml', 'avatar.txt', 'tips.txt', 'notice.json', 'jrrp.json', 'sentences.json', 'help.json', 'help/api.json']
export const sha256 = data => createHash('sha256').update(data).digest('hex')
const MAX_ARCHIVE = 128 * 1024 * 1024, MAX_UNPACKED = 512 * 1024 * 1024, MAX_FILE = 32 * 1024 * 1024
const pluginVersion = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8')).version.split('.').map(Number)
export const allowedResourceFile = (name, kind) => (kind === 'song-data' ? /\.(json|csv|ya?ml|txt|md)$/i : /\.(png|jpe?g|webp|txt|md)$/i).test(name)
  || /(?:^|\/)(?:LICENSE|LICENCE|COPYING|NOTICE|AUTHORS)$/i.test(name)

export class ResourceError extends Error {
  constructor(message) { super(message); this.name = 'ResourceError' }
}
function check(condition, message) { if (!condition) throw new ResourceError(message) }
export function resourceOptions(config = {}, env = process.env) {
  let base
  try { base = new URL(env.PHI_RESOURCE_BASE_URL || config.resourceBaseUrl || DEFAULT_RESOURCE_URL) }
  catch { throw new ResourceError('资源地址不是有效 URL。') }
  check(base.protocol === 'https:' || base.protocol === 'http:' && ['127.0.0.1', 'localhost', '[::1]'].includes(base.hostname), '资源地址须为 HTTPS（本机调试允许 HTTP）。')
...
export function validateResourcePath(value) {
  ...
  check(value.split('/').every(part => part && !part.startsWith('.') && !/[. ]$/.test(part) && !/^(?:con|prn|aux|nul|com\d|lpt\d)(?:\.|$)/i.test(part)), '资源包含不安全路径。')
  return value
}
async function download(baseUrl, relative, maxBytes, signal) {
  const url = new URL(validateResourcePath(relative).split('/').map(encodeURIComponent).join('/'), baseUrl), controller = new AbortController()
  try {
    const response = await fetch(url, { redirect: 'error', signal: AbortSignal.any([controller.signal, AbortSignal.timeout(120000), ...(signal ? [signal] : [])]) })
...
function json(buffer) {
  ...
  catch { throw new ResourceError('资源索引不是有效 JSON。') }
}
...
export function validateResourceIndex(index) {
  ...
  for (const pointer of index.releases) {
    ...
    if (pointer.game !== undefined) check(pointer.game?.version === pointer.version.split('-r')[0] && Number.isSafeInteger(pointer.game.code) && pointer.game.code > 0, '资源版本列表中的游戏版本无效。')
    if (pointer.packages !== undefined) check(Array.isArray(pointer.packages) && pointer.packages.length === 1 && pointer.packages[0] === 'song-data', '资源版本列表中的资源包种类无效。')
  }
  return index
}
export function validateManifest(manifest) {
  check(manifest?.schemaVersion === 1 && manifest.dataFormat === 'phi-info-v1' && RESOURCE_VERSION.test(manifest.version), '不支持的资源数据格式。')
  check(/^\d+\.\d+\.\d+$/.test(manifest.game?.version) && Number.isSafeInteger(manifest.game.code) && manifest.game.code > 0, '资源缺少游戏版本。')
  ...
  check(manifest.packages && typeof manifest.packages === 'object' && !Array.isArray(manifest.packages) && manifest.packages['song-data'], '资源清单缺少 song-data。')
  for (const [kind, pack] of Object.entries(manifest.packages)) {
    check(kind === 'song-data' && Array.isArray(pack?.archives) && pack.archives.length > 0 && pack.archives.length <= 256, '资源包种类或分包数量无效。')
    const paths = new Set()
    for (const archive of pack.archives) {
      validateResourcePath(archive?.path)
      check(archive.path.startsWith(`v/${manifest.version}/${kind}-`) && archive.path.endsWith('.zip') && !paths.has(archive.path), '资源 ZIP 路径无效或重复。')
      paths.add(archive.path)
      check(/^[a-f0-9]{64}$/.test(archive.sha256), '资源包 SHA-256 无效。')
      for (const [key, max] of [['bytes', MAX_ARCHIVE], ['unpackedBytes', MAX_UNPACKED], ['fileCount', 10000]]) {
...
export async function extractArchive(buffer, archive, destination, kind, seen = new Set()) {
  ...
  let zip
  try { zip = await JSZip.loadAsync(buffer) } catch { throw new ResourceError('资源 ZIP 无效。') }
  let total = 0, count = 0
  for (const entry of Object.values(zip.files)) {
    ...
    if (entry.dir) { await fs.mkdir(path.join(destination, name), { recursive: true }); continue }
    check(!seen.has(name.toLowerCase()), '资源分包文件重复。'); seen.add(name.toLowerCase())
...
export async function validateInfoDirectory(dir) {
  ...
  for (const name of ['DLC', 'oldInfo']) check((await fs.stat(path.join(dir, name)).catch(() => null))?.isDirectory(), `曲目元数据缺少 ${name} 目录。`)
...
async function readCachedJson(file) {
  try { return json(await fs.readFile(file)) }
  catch (error) {
...
export class ResourceRepository {
  ...
  async #resolveManifest() {
    const pointer = this.options.version === 'latest'
  ...
  async #installPackage(manifest, kind) {
    ...
    check(!(await fs.lstat(destination).catch(error => { if (error.code === 'ENOENT') return null; throw error })), '已有资源版本与清单不一致；请发布新版本号或先手动移走损坏缓存。')
    await fs.mkdir(path.dirname(destination), { recursive: true, mode: 0o700 })
    const stage = await fs.mkdtemp(path.join(path.dirname(destination), `.${kind}-`))
    try {
      const seen = new Set()
      for (const archive of pack.archives) await extractArchive(await download(this.options.baseUrl, archive.path, archive.bytes), archive, stage, kind, seen)
  ...
  async #ensure(refresh) {
    ...
    if (this.options.illustrations) {
      ...
      try { await illustrations.installAll({ refresh }) } finally { await illustrations.close() }
    }
    return { manifest, infoPath: path.join(release, 'song-data'), illustrationPath: path.join(this.root, 'illustrations'), cacheRoot: this.root, baseUrl: this.options.baseUrl }
  }

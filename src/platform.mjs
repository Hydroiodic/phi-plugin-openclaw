...
export class OpenClawPlatform {
  ...
  constructor({ dataRoot, logger = console, config = {}, mediaRoot } = {}) {
    ...
    this.PluginBase = class {
      ...
      setContext(name, isGroup = false, timeout = 120) { return contexts.set(this, name, isGroup, timeout) }
      /** @param {string} name @param {boolean} [isGroup] */
      finish(name, isGroup = false) { return contexts.finish(this, name, isGroup) }
    }
  }

  getBotConfig() { return { chromium_path: this.config.chromiumPath } }
  getPackageVersion() { return packageVersion }
  getBotNickname() { return 'OpenClaw' }
  /** @param {any} e */
  getAdapterName(e) { return e?.platform === 'qqbot' ? 'QQBot' : 'openclaw' }
  isBotReady() { return !this.closed && Boolean(this.dataRoot) }
  /** @param {any} message */
  toPlatformMessage(message) { return message }
  ...
  wrapEvent(e) {
    ...
    if (!e._replies) {
      ...
      Object.defineProperty(e, '_replyCount', {
        ...
        get: () => e._replies.count, set: value => { e._replies.count = value },
      })
    }
    e.msg ??= e.text || ''
  ...
  cloneEvent(e, overrides = {}) {
    ...
    Object.defineProperty(clone, '_replyCount', {
      enumerable: true, get: () => e._replies.count, set: value => { e._replies.count = value },
    })
  ...
  fromContext(ctx, deliver) {
    ...
    const e = this.wrapEvent({
      msg: ctx.commandBody || '', user_id: userId, userId, platform: channel,
      chatId, group_id: isGroup ? chatId : undefined, groupId: isGroup ? chatId : undefined,
      isGroup, isPrivate: !isGroup, chatType: isGroup ? 'group' : 'private',
      sender: { nickname: ctx.senderName || 'Phigros 玩家', user_id: userId, card: ctx.senderName || '' },
      self_id: account, rawSenderId: String(ctx.senderId), message_id: ctx.messageId,
      isMaster: (Array.isArray(this.config.admins) ? this.config.admins : []).includes(`${channel}:${account}:${ctx.senderId}`),
      is_admin: false, is_owner: false, _deliver: deliver, _signal: ctx.signal,
    })
    // QQ exposes no group roster here. Transfers resolve only known users,
    // isolated by channel and bot account.
    e._replies.track(this.redis.set(`phiPlugin:knownUser:${userId}`, JSON.stringify({
      channel, account, user_id: userId, nickname: ctx.senderName || 'Phigros 玩家',
    })))
    return e
  }
  ...
  async pickMember(e, target) {
    let raw = String(target ?? '').trim().replace(/^<@!?([^>]+)>$/, '$1').replace(/^\[CQ:at,qq=([^,\]]+)(?:,[^\]]*)?\]$/, '$1').replace(/^@/, '')
    if (!raw) return null
    ...
    for (const id of candidates) {
      ...
      } catch { this.logger.warn('忽略无法解析的用户缓存。') }
    }
    return null
  }
  ...
  /** @param {any} e @param {any} message */
  sendWithAt(e, message) { return this.reply(e, message) }
  /** @param {any} e */
  flush(e) { return this.wrapEvent(e)._replies.flush() }
  /** @param {any} message */
  async payload(message) { return this.payloadEncoder.encode(await this.prepareIllustrations(message)) }
  /** @template T @param {T} value @returns {Promise<T>} */
  async prepareIllustrations(value) { return this.illustrations ? this.illustrations.prepare(value) : value }
  /** @param {any} e @param {any} message */
  ...
  /** @param {string} dir */
  mkdirs(dir) { fs.mkdirSync(dir, { recursive: true, mode: 0o700 }); return true }
  ...
  /** @param {any} e */
  async uploadFile(e) { return this.reply(e, '请在插件数据目录的 backup 文件夹中读取备份文件。') }
  async restartBot() { return false }
  /** @param {any} e */
  hasContext(e) { return Boolean(this.contexts.get(e)) }
  /** @param {any} e */
  getContext(e) { return this.contexts.get(e) }
  close() {
...
export function createPlatform(options) { return new OpenClawPlatform(options) }
